...
class CustomOptionMenu(ctk.CTkFrame):
    ...
    #     # ─── CALCULATE DYNAMIC HEIGHT CONSTRAINTS ───
    #     # Determine how many items we are actually going to display
    #     items_count = min(len(self.filtered_values), self.max_results)

    #     # 38px is your row height. We multiply it by the items count.
    #     # We add 8px for the top/bottom internal frame padding.
    #     calculated_height = (items_count * 38) + 12

    #     # Optional capping: Ensure it never exceeds the absolute dropdown height maximum
    #     final_height = min(calculated_height, self.dropdown_height)
    ...
    #     # Change self.dropdown_height to your new final_height variable
    #     self.dropdown_window.geometry(f"{width}x{final_height}+{x}+{y}")

    #     # Wipe structural children layers out of container memory to handle fresh redraw safely
    #     if hasattr(self, "results_frame"):
    #         self.results_frame.destroy()
    ...
    #     # Prevent the panel context window layer from stealing active view focus
    ...
    #     # Safe escape route: auto-collapses dropdown window if user clicks elsewhere
